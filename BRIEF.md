...
Because a month has to be genuinely "in progress" for a projection to mean anything, the dataset also marks one month — the most recent one, Dec 2025 — as partial: it carries `monthStatus: "in-progress"` (every other month is `"actual"`) plus `daysElapsed: 18` and `daysInMonth: 31`. Its metric values represent month-to-date totals as of that partial point (count metrics scaled to ~58% of the full month that was already in the dataset; `onTimeRate`/`avgTransitDays` left as recorded, since they're rate/average reads, not counts to shrink) — this is what the projection logic scales up from.
...
– Risk level is derived by comparing the projected on-time rate to the same three thresholds as the regional status chips (≥95% On Target, 92–94.9% At Risk, <92% Below Target) — same language, same color convention, so it reads as an extension of a pattern the VP already knows rather than a new system.
...
– **New: Exception type breakdown chart** — a bar chart (Pareto-ordered, highest-volume type first) of exception event counts by type over the selected period, with a toggle to switch between "by month" (stacked bars, good for spotting a seasonal spike like weather in Q4) and "by type" (a single ranked bar per type, summed over the period, good for spotting a systemic issue like recurring no-shows). Sits alongside the existing two charts as a third chart in that row, or wraps to its own row on smaller screens.
  – Like the other two charts, "by month" respects the month picker: all 12 months when "All" is selected, just the one selected month's stacked bar otherwise — it isn't a separate, always-full-year view.
  – Charts always plot the actual recorded value for every month, including the in-progress month's true month-to-date figure. The "Projected" treatment is a KPI-card/regional-card affordance only (see below) — charts never silently substitute a paced estimate for a real data point.
– 4-up regional performance grid, one card per region. During the in-progress month, each regional card's status chip is joined by a second, visually subordinate "Projected: On Target / At Risk / Below Target" line — the current chip stays the source of truth for "where things stand today," the projection is clearly the secondary, forward-looking read.
...
– **New: Year toggle** — implemented as a "Compare to last year" checkbox next to the month picker (the simpler of the two options the brief allowed) that adds a second, visually distinct series (dashed line or lighter shade) to the on-time delivery chart and the exception type breakdown chart, showing the same period one year prior. This answers "are we better than last winter" directly on the chart rather than requiring a separate view. When YoY comparison is on, KPI cards optionally show "+X% vs. same month last year" alongside the existing month-over-month arrow, only when a specific month is selected — these are two different comparisons, labeled distinctly so they're not confused.
  – On the exception type chart's "by month" view, the prior-year overlay is a single dashed line of *total* exceptions per month (not a full second stacked breakdown by type) — six extra stacked series would make the chart unreadable. The "by type" view shows the full prior-year breakdown, since it's only 6 bars either way.
